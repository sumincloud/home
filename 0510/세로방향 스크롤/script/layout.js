$(document).ready(function(){
  ...
  $('nav li a').click(function(){
    ...
    //각 href속성값에 scrollTop값을 0으로 맞추면 해당 콘텐츠가 나오게 됨
    ...
    $(this).on('mousewheel',function(e){
      ...
      if(delta < 0) {
        ...
        if($(this).index() < boxMax) {
          ...
          //앞으로 갈때 내비게이션 색상변경 서식 ----------------
          let next = $(this).next().attr('id');

          //모든 메뉴서식을 제거하고
          $('nav li a').removeClass('on');
          //동일한 a값에 on클래스 적용
          $('a[href$='+next+']').addClass('on')
          //--------------------------------------------------
      ...
      // 마우스휠을 아래에서 위로 이동( 뒤에서 앞으로 이동)
        ...
        // 첫번째 article보다 더 위로 이동하려고 하는 경우 알림창 출력
          ...
          //화면 이동 0.3초(300)
          $("html,body").stop().animate({scrollTop : moveTop + "px"}, 500);
